...
def remove_background(image):
    ...
    lower_green = np.array([40, 40, 40])  # Nilai batas rendah untuk warna hijau dalam format HSV
    upper_green = np.array([80, 255, 255])  # Nilai batas tinggi untuk warna hijau dalam format HSV
    mask = cv2.inRange(hsv, lower_green, upper_green)
    result = cv2.bitwise_and(image, image, mask=mask)
    return result

# Baca gambar 
original = cv2.imread('Tambahan/botol.jpg')
